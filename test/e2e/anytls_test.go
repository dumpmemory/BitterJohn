...
func startBitterJohnAnyTLSServer(t *testing.T, password string) string {
	...
	srv, err := anytlsserver.New(anyTLSTestTLSContext(t), direct.SymmetricDirect)
	if err != nil {
		t.Fatal(err)
	}
...
func anyTLSTestTLSContext(t *testing.T) context.Context {
	t.Helper()
	return anytlsserver.WithTLSConfig(context.Background(), anyTLSTestTLSConfig(t))
}

func anyTLSTestTLSConfig(t *testing.T) *tls.Config {
	t.Helper()
